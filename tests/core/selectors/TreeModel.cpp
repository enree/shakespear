...
QModelIndex
TreeModel::index(int row, int column, const QModelIndex& index) const
{
...
bool TreeModel::hasChildren(const QModelIndex& index) const
{
    TreeItem* item = itemForIndex(index);
    return item == nullptr
           || item->hasChildren(); // Is the first condition true?
}
    ...
    const QModelIndex& index, const QVariant& data, int role)
{
    TreeItem* item = itemForIndex(index);
    bool res
        = item != nullptr ? item->setData(index.column(), data, role) : false;
    if (res)
    {
